...
// ToolChain 代表特定版本的SDK开发工具（如Go、Node.js等），包含版本号、安装路径、别名等属性
type ToolChain struct {
	...
	InstallDir  string            `json:"install_dir"`  // tool 安装目录路径
	ActiveEnv   map[string]string `json:"active_env"`   // 激活时设置的额外环境变量
	InitVersion string            `json:"init_version"` // 首次初始化时确认下载的版本号
