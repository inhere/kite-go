...
func TestParseVersionSpec(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.input, func(t *testing.T) {
			...
			if result.SDK != tc.expected.SDK {
				t.Errorf("Expected SDK %q, got %q", tc.expected.SDK, result.SDK)
			}
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			for i, expected := range tc.expected {
				if result[i].SDK != expected.SDK {
					t.Errorf("Expected SDK %q at index %d, got %q", expected.SDK, i, result[i].SDK)
				}
